...
class Vinyle extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .get(
        "https://api.discogs.com/users/ausamerika/collection/folders/0/releases"
      )
      .then((response) => {
    ...
    return (
      <div className="vinyle" style={{ maxWidth: "500px", margin: "0 auto" }}>
        <Button
          ...
          onClick={() => this.choisirAleatoirementUnVinyle(tbVinyles)}
        >
          Choisir un autre vinyle
        </Button>
        <Card style={{ marginTop: "30px" }}>
          <Card.Body>
            <Card.Title>{this.state.vinyle.title}</Card.Title>
            <Card.Subtitle style={{ marginBottom: "20px" }}>
              {this.state.vinyle.year}
            </Card.Subtitle>
